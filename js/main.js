document.addEventListener("DOMContentLoaded", function () {
  ...
  // Loan Gallery Slider
  if (document.querySelector(".swiper--loan")) {
    new Swiper(".swiper--loan", {
      effect: "coverflow",
      loop: true,
      centeredSlides: true,
      slidesPerView: 1.8,
  ...
  // Lloyd Form Submission
  const formLloyd = document.querySelector(".lloyd__form-main");
  const lloydSection = document.querySelector(".lloyd");
  const successSection = document.querySelector(".lloyd-success");

  if (formLloyd && lloydSection && successSection) {
    formLloyd.addEventListener("submit", function (e) {
      e.preventDefault();

      lloydSection.classList.add("lloyd--hide");
      successSection.classList.add("lloyd-success--active");
